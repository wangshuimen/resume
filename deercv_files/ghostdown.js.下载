...
var ghostdown_run=function($, ShowDown, CodeMirror) {
	"use strict";
	...
	$(function() {
        ...
        */
        ...
        window.ghost_editor_instance = editor;
        ...
        //[luofei] 设置隐藏区域可见
        function updateHidenn(){
            $(document.getElementsByClassName('rendered-markdown')[0]).find('p:contains("[hidden]")').html("<div>输入密码可见</div>");
        }

        //[luofei] 设置password的现实
        function updatePassword(){
             var password_box = $(document.getElementsByClassName('rendered-markdown')[0]).find('p').filter(function(){
                 return /\[password\][\s\S]*?\[\/password\]/.test($(this).text())
            });
            password_box.addClass("password_border");
        }
		...
		function updateImagePlaceholders(content) {
			...
			$(imgPlaceholders).each(function( index ) {
				...
				(function(){
					...
					self.dropzone({
                        //上传图片的地址
                        url: api_base+"resume/uploadPic?token="+localStorage.getItem('token'),
		...
		function updatePreview() {
            ...
            // a = filterXSS(a);
			preview.innerHTML = filterXSS(converter.makeHtml(value));

            preview.innerHTML = preview.innerHTML.replace('[password]','<div class="password_border">');
            preview.innerHTML = preview.innerHTML.replace('[/password]','</div>');
            updateHidenn();
            //updatePassword();
